 ...
 * Est une implémentation du pattern Stratégie : "Répétition".
 *
 */
public class AlgorithmeExtensionRepetition implements AlgorithmeExtension {
     ...
     * Cette méthode suit les rêgles d'extension "par répétition".
     *
     ...
     *
     */
    private ArrayList<Cellule> getCellulesSansExtension(Grille grille, int[] position){
        if(position.length != 2){
            throw new IllegalArgumentException("La position demandé ne s'inscrit pas dans un espace de dimension 2.");
        }
        ...
        cellulesALaPosition.removeIf(cellule -> (cellule.getPosition()[0] != position[0] || cellule.getPosition()[1] != position[1]));
